// ============================================
// CIRO Intel Agent — Multi-Source Signal Fusion
// ============================================
// Pipeline:
//  1. New case received → GPS locked
//  2. Cluster scan → Firestore nearby cases (30 min window)
//  3. Weather check → Open-Meteo API (free, no key)
//  4. Map context → Google Places / demo fallback
//  5. Social signals → Firestore social_signals_demo
//  6. Confidence score calculated from all signals
//  7. Crisis event created if confidence ≥ 65%
//  8. Orchestrator + Emergency Dashboard alerted
// ============================================
...
export async function runIntelAgent(
  ...
  if (!belowThreshold) {
    ...
    // ── Below threshold — still report to Emergency Dispatch ──────────
    // Every case, even a hand cut, is reported with its confidence score
    // so the dispatch team has full situational awareness.

    // Determine risk label for dispatch
    const riskLabel =
    ...
    // Always notify Emergency Dispatch Hub — every case, every time
    await addIntelligenceLog({
      caseId,
      agentName: "Orchestrator",
